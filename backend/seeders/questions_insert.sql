...
-- LAS questions
    ...
    -- SEC questions
    ("In Unix il comando passwd si usa per verificare la robustezza della password",0,2),
    ("Nell'autenticazione passiva Prover e Verifier confrontano la conoscenza di un segreto",1,2),
    ("Una password che utilizza tutti i tipi di caratteri è sempre più robusta di una che utilizza solo una categoria (es. lettere minuscole)",0,2),
    ("I sistemi a sfida e risposta sono tipicamente implementati condividendo un segreto come ad esempio una chiave simmetrica.",null,2),
    ("Tra i fattori di autenticazione c'è qualcosa che si possiede materialmente come un Pin o una Password",1,2),
    ("I due paradigmi fondamentali per il controllo dell'accesso sono DAC (Discretionary access control) e TAC (Tertiary access control)",0,2),
    ("Ogni file del filesystem Unix è protetto da un set di permessi codificati in 12 bit",1,2),
    ("Le ACL sono liste associate ad ogni soggetto del sistema",0,2),
    ("Nel modello RBAC (Role-based access control) i permessi sono assegnati ai ruoli",1,2),
    ("Le ACL e le Capabilities sono la stessa cosa",null,2),
    ("Nel test di Kasiski si impiega la fattorizzazione delle distanze e la scelta di quelle con un fattore comune",null,2),
    ("Nell'attacco dei cifrari a sostiuzioni il fatto che alcune lettere siano più frequenti di altri nel linguaggio naturale non ha nessuna importanza",0,2),
    ("Nei cifrari a sostituzione polialfabetica le frequenze di un carattere cifrato derivano da contributi di diversi caratteri in chiaro",1,2),
    ("La crittografia permette di proteggere le quattro proprietà di sicurezza dell'informazione: riservatezza, integrità, autenticità e disponibilità",0,2),
    ("Nei cifrari a trasposizione le statistiche dei digrammi e trigrammi permettono di dedurre la dimensione della tabella di cifratura",1,2),
    ("DH e RSA hanno scopi differenti: quello di RSA é di essere molto più veloce nella fase di cifratura/decifrazione",0,2),
    ("DH e RSA hanno scopi differenti: quello di DH è scambiare una chiave condivisa tra due parti",1,2),
    ("Data chiave pubblica (e, n) e chiave privata (d, n) la cifratura consiste nel calcolare: c = m^e mod n",1,2),
    ("Diffie-Hellmann è uno schema di cifratura a chiave simmetrica",0,2),
    ("Gli algoritmi di cifratura a blocchi sono così definiti perchè si arrestano non appena la cifrazione è considerata sicura",0,2),
    ("La cifratura dei dischi protegge da qualsiasi tentativo di esfiltrazione dei dati",0,2),
    ("Le chiavi di autenticazione usate da Secure Boot sono aggiornabili senza interruzioni di servizio",null,2),
    ("La collocazione di sistemi in cloud ha unicamente effetti positivi sulla sicurezza",0,2),
    ("Esistono tre tipi fondamentali di firewall Packet filter, Application-level gateway, Circuit-level gateway",1,2),
    ("È possibile danneggiare fisicamente un sistema attraverso una porta USB",1,2),
    ("Un Intrusion Detection System può bloccare un attacco in corso",0,2),
    ("Il tasso di 'falsi positivi' non è un parametro importante per la qualità di un IDS",0,2),
    ("Un vantaggio degli Host-based IDS è che possono classificare più accuratamente il rischio associato a un pacchetto di rete",0,2),
    ("Suricata può funzionare sia da IDS che da IPS",null,2),
    ("Il kernel di Linux permette di porre sotto monitoraggio l'invocazione di ogni system call",null,2),
    ("802.1x è uno standard di autenticazione utilizzato nelle connessioni fisiche",1,2),
    ("Gli attacchi passivi non modificano i dati in transito",1,2),
    ("X.509 è la versione di SSL più utilizzata",null,2),
    ("Le botnet sono reti di computer infetti chiamati zombie",1,2),
    ("Appropriarsi dell'indirizzo di un host può permettere di attaccarlo di riflesso",null,2),
    ("Con una Remote File Inclusion possiamo recuperare file interni al web server",1,2),
    ("Il registro EIP in x86 salva il valore dell'indirizzo di ritorno da una funzione",null,2),
    ("Code Injection si verifica quando dell'input non sanitizzato viene interpretato come codice",1,2),
    ("La strcpy in linguaggio C non è una funzione a rischio di generare vulnerabilità di buffer overflow",0,2),
    ("I canarini sono un meccanismo di protezione del kernel linux per segnalare un overflow in memoria",0,2),
    ("FIDO alliance è un sistema di generazione degli OTP",0,2),
    ("Lo scopo del TOTP è quello di forzare l'utente a cambiare periodicamente la password",0,2),
    ("Tra i fattori di autenticazione c'è qualcosa che si possiede materialmente come un Pin o una Password",0,2),
    ("Tra i fattori di autenticazione c'è qualcosa che si conosce (Password, PIN)",1,2),
    ("In Unix il comando passwd si usa per verificare la robustezza della password",0,2),
    ("Il SUID è il bit che permette di lanciare con sudo il programma su cui è settato",0,2),
    ("I bit di autorizzazione standard nel filesystem Unix sono di 3 tipi R,W,X (read,write,execute)",1,2),
    ("Il controllo dell’accesso è decidere se un soggetto può eseguire una specifica operazione su di un oggetto",1,2),
    ("Nel modello RBAC (Role-based access control) i permessi sono assegnati ai ruoli",1,2),
    ("I due paradigmi fondamentali per il controllo dell'accesso sono DAC (Discretionary access control) e TAC (Tertiary access control)",null,2),
    ("Nei cifrari a sostituzione polialfabetica le frequenze di un carattere cifrato derivano da contributi di diversi caratteri in chiaro",1,2),
    ("Nel cifrario con sostituzione monoalfabetica sull'alfabeto inglese lo spazio delle chiavi è grande 26!",1,2),
    ("I cifrari classici possono essere impiegati per proteggere riservatezza e autenticità",0,2),
    ("Nell'attacco dei cifrari a sostiuzioni il fatto che alcune lettere siano più frequenti di altri nel linguaggio naturale non ha nessuna importanza",0,2),
    ("La proprietà di diffusione misura il grado in cui le proprietà statistiche degli elementi del testo cifrato vengono sparse sugli elementi del testo in chiaro",null,2),
    ("La proprietà collision-free degli hash significa che non si può trovare una coppia di documenti con lo stesso hash",1,2),
    ("Gli algoritmi di cifratura a blocchi cifrano in sequenza i frammenti in cui viene suddiviso il testo in chiaro",1,2),
    ("Il miglior attacco a RSA è la ricerca dei fattori del modulo",1,2),
    ("Nel modello Infrastrutturale della certificazione delle chiavi pubbliche l'autenticità della chiave pubblica è data da un soggetto terzo fidato che emette la certificazione",1,2),
    ("CBC sta per Cipher Block Chaining e consiste nel Cifrare un blocco modificandolo col contributo del blocco cifrato precedente",null,2),
    ("L'approccio default deny su firewall significa che tutto il traffico viene bloccato",0,2),
    ("La collocazione di sistemi in cloud migliora (generalmente) la disponibilità dei servizi",1,2),
    ("La collocazione di sistemi in cloud ha unicamente effetti positivi sulla sicurezza",0,2),
    ("Esistono tre tipi fondamentali di firewall Packet filter, Application-level gateway, Circuit-level gateway",1,2),
    ("Measured Boot si riferisce a un processo generale, che tipicamente usa un TPM come hardware root of trust",1,2),
    ("Il controllo dell'integrità dei file è uno dei metodi usati dagli HIDS",null,2),
    ("Suricata può funzionare sia da IDS che da IPS",null,2),
    ("Il tasso di 'falsi positivi' non è un parametro importante per la qualità di un IDS",0,2),
    ("Il kernel di Linux permette di porre sotto monitoraggio l'invocazione di ogni system call",1,2),
    ("I sistemi SIEM raccolgono dati che devono essere nativamente omogenei per poter essere confrontati e aggregati",null,2),
    ("Il Command and Control è il computer incaricato di gestire le comunicazioni con gli elementi di una botnet",1,2),
    ("Appropriarsi dell'indirizzo di un host può permettere di attaccarlo di riflesso",1,2),
    ("HTTPS è HTTP su di un canale di comunicazione cifrato",1,2),
    ("Gli attacchi passivi non modificano i dati in transito",1,2),
    ("Lo sniffing può compromettere la riservatezza dei dati",1,2),
    ("ASLR è un meccanismo di protezione del kernel linux per randomizzare gli spazi di memoria",null,2),
    ("Inserire un ritardo di pochi secondi tra due login errate non è una misura efficace per mitigare gli attacchi brute force",0,2),
    ("Con una Remote File Inclusion possiamo recuperare file interni al web server",1,2),
    ("Nelle time based sql-injection è importante il tempo di computazione della query sql",1,2),
    ("Se un sito è protetto da TLS non è possibile eseguire una SQL Injection",null,2);