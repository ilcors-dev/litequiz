...
-- LAS questions
    ...
    ("Le ACL e le Capabilities sono la stessa cosa",null,2,1),
    ("Nel test di Kasiski si impiega la fattorizzazione delle distanze e la scelta di quelle con un fattore comune",null,2,1),
    ...
    ("Le chiavi di autenticazione usate da Secure Boot sono aggiornabili senza interruzioni di servizio",null,2,1),
    ...
    ("Suricata può funzionare sia da IDS che da IPS",null,2,1),
    ("Il kernel di Linux permette di porre sotto monitoraggio l'invocazione di ogni system call",null,2,1),
    ...
    ("X.509 è la versione di SSL più utilizzata",null,2,1),
    ("Le botnet sono reti di computer infetti chiamati zombie",1,2,1),
    ("Appropriarsi dell'indirizzo di un host può permettere di attaccarlo di riflesso",null,2,1),
    ("Con una Remote File Inclusion possiamo recuperare file interni al web server",1,2,1),
    ("Il registro EIP in x86 salva il valore dell'indirizzo di ritorno da una funzione",null,2,1),
    ...
    ("CBC sta per Cipher Block Chaining e consiste nel Cifrare un blocco modificandolo col contributo del blocco cifrato precedente",null,2,1),
    ...
    ("Il controllo dell'integrità dei file è uno dei metodi usati dagli HIDS",null,2,1),
    ("Suricata può funzionare sia da IDS che da IPS",null,2,1),
    ("Il tasso di 'falsi positivi' non è un parametro importante per la qualità di un IDS",0,2,1),
    ("Il kernel di Linux permette di porre sotto monitoraggio l'invocazione di ogni system call",1,2,1),
    ("I sistemi SIEM raccolgono dati che devono essere nativamente omogenei per poter essere confrontati e aggregati",null,2,1),
    ...
    ("ASLR è un meccanismo di protezione del kernel linux per randomizzare gli spazi di memoria",null,2,1),
    ...
    ("Se un sito è protetto da TLS non è possibile eseguire una SQL Injection",null,2,1),
